...
func (rm *RelayManager) ConnectToRelay(relay *RelayCandidate) error {
	...
	sessionID := fmt.Sprintf("relay-session-%s-%d", rm.dhtPeer.NodeID(), time.Now().Unix())
...
func (rm *RelayManager) sendKeepalives() {
	keepaliveInterval := rm.config.GetConfigDuration("relay_keepalive_interval", 30*time.Second)
	...
	for {
		...
		case <-ticker.C:
			...
			stream, err := conn.OpenStreamSync(rm.ctx)
			if err != nil {
				rm.logger.Warn(fmt.Sprintf("Keepalive failed: %v", err), "relay-manager")
				continue
			}

			pingMsg := CreatePing(sessionID, "keepalive")
			if msgBytes, err := pingMsg.Marshal(); err == nil {
				stream.Write(msgBytes)
			}
			stream.Close()
...
func (rm *RelayManager) periodicRelayEvaluation() {
	for {
		...
		case <-rm.evaluationTicker.C:
			...
			maxAge := rm.config.GetConfigDuration("relay_candidate_max_age", 10*time.Minute)
